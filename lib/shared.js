//Marked import.  
//This is dumb but I don't want there to be any more burden to the implementation developer than including the shared.js tag
  ...
  var timers = {};
	...
	var exports = {
		trimTitle : function(str, optionalLength) {
			...
			//Note that we have to be able to assume the API doesn't get unsafe html stored in it.
			//We're now only removing tags that shouldn't be shown in the text as a title.  
			//It is a goal to allow only italic, bold, underline, and so on.  
			//Note also that there's a danger in unclosed tags for the tags we do allow.  
			//On the whole this is a mess and a bad idea.  Eventually this might be ditched in favor of the api
			//returning a title_text.  

			//Problem is I really like how the title trimming gives each implementation an example of
			//using formatting helper functions.  Maybe that can be superseded by the markdown preview view.  
			str = str.replace(/<(p|a|td|code|pre|table|ol|li)>/g, "");
			str = str.replace(/<\/(p|a|td|code|pre|table|ol|li)>/g, "");
			if(!optionalLength) {
